...
- 2026-06-24 — **Sprint Report Column Streamlining:** Streamlined table structures in the sprint report. Removed "Phase", "Status", "Scheduled tasks", and "Unscheduled" columns from the Delivery Items (Epics) table. Removed the "QA" column from the Epic Quality Report table, dynamically adjusting headers, separator alignments, data cells, and summary lines. Updated unit tests and skill runbooks accordingly.
- 2026-06-24 — **Sprint Report Streamlining:** Condensed the sprint report layout to only include Executive Summary, Delivery Items (Epics), and the Epic Quality Report summary table. Removed timeline breakdown, detailed per-epic member bug lists, team tasks plans, data quality flags, schedule delas, and recommended actions sections; updated all tests, documentation, and agent specifications accordingly, while keeping the epic-estimation CLI completely untouched.
- 2026-06-24 — **Sprint Report Refactoring:** Renamed 'Bug fix effort' to 'Epic Quality Report' and reordered columns to place the total 'Bugs' count column next to the epic title before team hours. Refactored 'Delivery items (Epics)' table to display start/end dates for each delivery platform (Backend, Web, Mobile) and the computed Final Release date; updated corresponding pipelines, rendering logic, templates, test suites, and skill runbooks.
